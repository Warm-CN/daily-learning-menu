...
独立的 `/trends` 页面提供分时、日线和周线分析，可按项目筛选。分时实时显示当前计时，日线和周线以完成会话生成开高低收 K 线，并用下方柱形显示包含补录的统计总时长。

学习看板首页可以逐条记录当天知识点，“知识库”标签支持按月份、学科和关键词回顾历史，日历会标出每天的收获数量。知识点为账号私有数据，并包含在 JSON 备份中。

首页还支持一个云端目标日期倒计时；名称和日期会随账号在设备间同步，但不会写入 JSON 学习数据备份，因此导入或恢复学习备份不会改变倒计时设置。
...
在原本地版统计页导出 JSON，登录服务器版后在“统计”页选择“导入备份”。导入前的云端数据会保留为一个可恢复快照。旧格式备份不含 `knowledgePoints` 时会保留账号已有知识点；服务器版新导出的备份可以完整恢复知识库。账号的目标日期倒计时不包含在 JSON 中，导入和恢复均不会覆盖它。
