...
let selectedProject=null,selectedMode='countup',weekChart=null,pieChart=null,todayChart=null,calendarDate=new Date(),timerTick=null,timerSync=null,timerClock=null,finishing=false;
...
function timerClockNow(){return typeof performance!=='undefined'&&performance.now?performance.now():Date.now()}
function syncTimerClock(){const timer=state.timer;if(!timer){timerClock=null;return}const elapsed=Math.max(0,Number(timer.elapsedSeconds)||0),now=timerClockNow();if(timer.phase!=='running'){timerClock={sessionId:timer.sessionId,phase:timer.phase,elapsedSeconds:elapsed,anchoredAt:now};return}if(!timerClock||timerClock.sessionId!==timer.sessionId||timerClock.phase!=='running'){timerClock={sessionId:timer.sessionId,phase:'running',elapsedSeconds:elapsed,anchoredAt:now};return}const localElapsed=timerClock.elapsedSeconds+(now-timerClock.anchoredAt)/1000;if(Math.abs(elapsed-localElapsed)>2)timerClock={sessionId:timer.sessionId,phase:'running',elapsedSeconds:elapsed,anchoredAt:now}}
function currentTimerElapsedSeconds(){const timer=state.timer;if(!timer)return 0;if(timer.phase!=='running')return Math.max(0,Number(timer.elapsedSeconds)||0);if(!timerClock||timerClock.sessionId!==timer.sessionId||timerClock.phase!=='running')syncTimerClock();return Math.max(0,timerClock.elapsedSeconds+(timerClockNow()-timerClock.anchoredAt)/1000)}
...
function timerDisplaySeconds(){const seconds=currentTimerSeconds();return state.timer?.mode==='countdown'?Math.ceil(seconds):Math.floor(seconds)}
function renderTimerDisplay(){$('timerDisplay').textContent=formatDuration(timerDisplaySeconds())}
function nextTimerDisplayDelay(){const seconds=currentTimerSeconds(),fraction=seconds-Math.floor(seconds);if(state.timer?.mode==='countdown')return Math.max(30,Math.min(1010,(fraction||1)*1000+10));return Math.max(30,Math.min(1010,(1-fraction||1)*1000+10))}
function scheduleTimerTick(){clearTimeout(timerTick);const timer=state.timer;if(!timer||timer.phase!=='running')return;timerTick=setTimeout(async()=>{renderTimerDisplay();if(timer.mode==='countdown'&&currentTimerSeconds()<=0&&!finishing){finishing=true;try{await syncTimer();await loadData();toast('倒计时完成，学习记录已保存')}finally{finishing=false}return}scheduleTimerTick()},nextTimerDisplayDelay())}
function renderTimer(){const timer=state.timer,locked=Boolean(timer);syncTimerClock();$('countupMode').classList.toggle('active',(timer?.mode||selectedMode)==='countup');$('countdownMode').classList.toggle('active',(timer?.mode||selectedMode)==='countdown');$('durationRow').hidden=(timer?.mode||selectedMode)!=='countdown';$('timerProject').textContent=timer?`${timer.projectName} · ${timer.phase==='running'?'进行中':'已暂停'}`:(state.projects.find(p=>p.id===selectedProject)?.name||'请选择项目');renderTimerDisplay();$('startTimer').hidden=timer?.phase==='running';$('startTimer').textContent=timer?.phase==='paused'?'▶ 继续':'▶ 开始';$('pauseTimer').hidden=timer?.phase!=='running';$('stopTimer').hidden=!timer;$('resetTimer').hidden=!timer;[$('countupMode'),$('countdownMode'),$('durationMinutes'),$('manageProjects')].forEach(el=>el.disabled=locked);scheduleTimerTick();clearInterval(timerSync);if(timer)timerSync=setInterval(syncTimer,5000)}
